// Z Pre-pass Shader - Optimized for depth-only rendering with alpha testing
...
fn fs_main(in: FragmentInput) -> @location(0) vec4f {
    ...
    // Sample texture and perform alpha test
    let textureColor = textureSample(textureArray, textureSampler, in.uv, layer);
    
    if (textureColor.a < 0.9) {
        discard;
    }

    // Z pre-pass doesn't need to output color, just depth
    // The depth is automatically written by the GPU
    return vec4f(0.0, 0.0, 0.0, 1.0);  // Dummy output
}